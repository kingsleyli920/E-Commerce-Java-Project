				...
				<div style="margin:0 auto; margin-top:10px;width:950px;">
					...
					<table class="table table-bordered">
						...
						<tbody>
							<tr class="success">
								<th colspan="5">
									<a href="${pageContext.request.contextPath}/OrderServlet?method=findOrderByOid&oid=${o.oid}" >订单编号: ${o.oid}</a>
									总金额: ${o.total}元
									<c:if test="${o.state == 1}">未付款</c:if>
									<c:if test="${o.state == 2}">未发货</c:if>
									<c:if test="${o.state == 3}">待签收</c:if>
									<c:if test="${o.state == 3}">已关闭</c:if>
								</th>
							</tr>
							<tr class="warning">
								<th>图片</th>
